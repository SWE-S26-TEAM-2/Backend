...
class SocialLinkService:
    ...
    @staticmethod
    def get_social_links(db: Session, current_user: User) -> dict:
        """
        ...
        """
        links = SocialLinkRepository.get_by_user_id(db, current_user.user_id)

        return {
            "success": True,
            "data": {
                "social_links": [
                    {"platform": link.platform, "url": link.url} for link in links
                ],
            },
        }

    @staticmethod
    def update_social_links(db: Session, current_user: User, data) -> dict:
        """
        ...
        """
        SocialLinkRepository.delete_by_user_id(db, current_user.user_id)
        SocialLinkRepository.create_many(db, current_user.user_id, data.social_links)

        # Re-fetch to get the DB-generated IDs
        updated = SocialLinkRepository.get_by_user_id(db, current_user.user_id)
        ...
        return {
            ...
            "data": {
                "social_links": [
                    {"platform": link.platform, "url": link.url} for link in updated
                ],
            },
        }
