...
@router.put("/me/avatar", status_code=status.HTTP_200_OK, response_model=AvatarResponse)
def upload_avatar(
...
@router.put("/me/cover", status_code=status.HTTP_200_OK, response_model=CoverResponse)
def upload_cover(
    file: UploadFile = File(...),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
